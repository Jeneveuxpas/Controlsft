...
The teacher/student A-F set initializes an unconditioned full-model student from the fixed Part16 condition teacher.
A and B use same-layer L1 alignment at block 24 and differ only by the student MLP projector. C-F use block 16→34
cosine alignment: C shares one timestep, D independently samples and sorts high/low for student/teacher, and E
assigns independent `s` to an expected 10% of student video tokens while its teacher uses `min(t,s)`. B→C changes both
layer selection and loss type, so it is not a pure layer-only comparison. All `/workspace/...` paths are
environment-specific templates. F keeps student `t` and restricts the teacher gap to `U(0,0.2)`.
