...
def test_prefixed_reference_tokens_restore_generated_target_before_decode() -> None:
    token_values = torch.tensor([20.0, 21.0, 22.0, 10.0, 11.0])
    attention_mask = torch.arange(25.0).reshape(1, 5, 5)
    state = LatentState(
        latent=token_values.reshape(1, 5, 1),
        denoise_mask=torch.tensor([0.0, 0.0, 0.0, 1.0, 1.0]).reshape(1, 5, 1),
        positions=token_values.reshape(1, 1, 5, 1),
        clean_latent=token_values.reshape(1, 5, 1),
        attention_mask=attention_mask,
    )

    output = ValidationRunner._move_prefixed_video_references_to_suffix(state, target_len=2)

    order = torch.tensor([3, 4, 0, 1, 2])
    assert torch.equal(output.latent.flatten(), token_values.index_select(0, order))
    assert torch.equal(output.clean_latent.flatten(), token_values.index_select(0, order))
    assert torch.equal(output.positions.flatten(), token_values.index_select(0, order))
    assert torch.equal(output.denoise_mask.flatten(), torch.tensor([1.0, 1.0, 0.0, 0.0, 0.0]))
    expected_attention = attention_mask.index_select(1, order).index_select(2, order)
    assert torch.equal(output.attention_mask, expected_attention)

    # LatentTools.clear_conditioning() keeps the first target_len tokens.
    assert torch.equal(output.latent[:, :2].flatten(), torch.tensor([10.0, 11.0]))
